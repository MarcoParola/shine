...
def main(cfg):
    ...
    for idx in range(dataset.len()):
        ...
        if cfg.dataset.view == 'both' or cfg.dataset.view == 'actual':
            try:
                with open(os.path.join( cfg.dataset.path, cfg.dataset.yolo_dataset_folder, dataset.get_file_name_by_id(idx) +'.txt'), "r") as f:
...
#######################################################################################
        ...
        if cfg.dataset.view == 'both' or cfg.dataset.view == 'predicted':
            file_name = dataset.get_file_name_by_id(idx)
            csv_boxes_path = os.path.join(cfg.output.path, cfg.output.predicted_merged_boxes_folder)
            create_dir(csv_boxes_path)
